...
/**
 * Modal para listar produtos de um fornecedor específico.
 * - Mantém visual neutro para não interferir no layout existente.
 * - Filtra os produtos SEMPRE no client pelo fornecedor selecionado.
 *
 * Props:
 * - open (boolean)
 * - onClose () => void
 * - fornecedor: { id: number|string, nome?: string, nome_fantasia?: string }
 */
export default function ProdutosDoFornecedorModal({ open, onClose, fornecedor }) {
  ...
  const fornecedorId = useMemo(() => {
    // Garante número para comparação robusta
    const raw =
      fornecedor?.id ??
  ...
  // Busca (uma vez por abertura) a lista completa e FILTRA no client
  useEffect(() => {
    let cancelado = false;
    async function carregar() {
      ...
      setErro("");
      try {
        // 1) Tenta pegar já filtrado se seu backend suportar (?fornecedorId=)
        // 2) Mas independentemente do que vier, vamos filtrar no client para garantir.
        let lista;
        try {
        ...
        // Normaliza e filtra no client (sempre)
        const normId = (v) => {
          const n = Number(v);
          return Number.isFinite(n) ? n : null;
        };
        const isDoFornecedor = (p) => {
          // Tentativas de campos comuns no seu backend
        ...
        // Mapeia para campos neutros sem alterar layout externo
