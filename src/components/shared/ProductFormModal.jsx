...
/**
 * Modal de cadastro/edição de produto (opaco, sem transparência).
 * Props:
 * - open: boolean
 * - onClose: () => void
 * - onSubmit: (produtoForm) => void
 * - initialData?: { id, nome, fornecedor, preco, estoque }
 * - fornecedores: array de fornecedores [{id, nome_fantasia, nome}]
 */
export default function ProductFormModal({ open, onClose, onSubmit, initialData, fornecedores = [] }) {
  ...
  useEffect(() => {
    if (open) {
      setForm({
        ...
        id_fornecedor: "", // sempre force seleção ao abrir
  ...
  function handleSubmit(e) {
    ...
    onSubmit({
      // Campos mínimos + extras que a tela possui
      produto: form.nome.trim(),
      id_fornecedor: Number(form.id_fornecedor),
      valor_produto: precoNum,
      estoque: estoqueNum, // OBS: se o backend não persistir estoque, tudo bem
    });
  }
  ...
  return (
    <div
      ...
      style={{ backgroundColor: "#0F0F10" }} // backdrop sólido
    >
